...
def get_wallets():
    return list(wallets_collection.find({}, {"_id": 0, "source": 0, "method": 0, "source_values": 0}))


@router.get("/wallets/debug")
def get_wallets_debug():
    wallets = list(wallets_collection.find({}, {"_id": 0}))
    return {
        "total": len(wallets),
        "wallets": wallets,
    }
...
@router.get("/wallets/debug/{wallet_id}")
def get_wallet_debug(wallet_id: str):
...
def get_wallet(wallet_id: str):
    wallet = wallets_collection.find_one({"id": wallet_id}, {"_id": 0, "source": 0, "method": 0, "source_values": 0})
    if not wallet:
        raise HTTPException(status_code=404, detail="Billetera no encontrada")
    return wallet
